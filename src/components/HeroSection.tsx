        ...
        const renderParticles = () => {
          ...
          for (let i = 0; i < particles.length; i++) {
            for (let j = i + 1; j < particles.length; j++) {
              ...
              if (dist < 72) {
                const lineAlpha = (1 - dist / 72) * 0.025;
                ctx.beginPath();
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="absolute -top-32 left-1/2 -translate-x-1/2 w-[700px] h-[500px] bg-emerald-500/5 rounded-full blur-[160px]" />
        <div className="absolute top-1/3 -left-32 w-[450px] h-[450px] bg-sky-500/[0.025] rounded-full blur-[160px]" />
      ...
      <div className="absolute inset-0 pointer-events-none z-0 overflow-hidden hidden lg:block">
        {FLOATING_DATA_NODES.map((node, i) => (
          <div
            key={i}
            className="hero-parallax-data hero-system-element absolute bg-zinc-950/20 px-2.5 py-1 text-[9px] font-mono tracking-tighter text-emerald-500/35 border-l border-emerald-500/20 animate-float"
            style={{
              top: node.top,
              left: node.left,
              right: node.right,
      ...
      <div className="relative z-10 max-w-5xl mx-auto text-center my-auto flex flex-col items-center">
        ...
        <div
          ...
          ref={taglineRef}
          className="hero-system-element inline-flex items-center gap-2 px-1 py-1 story-kicker mb-6 opacity-0 will-change-transform"
        >
        ...
        <h1
          id="hero-app-title"
          ref={titleRef}
          className="text-4xl sm:text-6xl md:text-7xl font-bold tracking-tight text-white mb-4 flex items-center justify-center select-none will-change-transform"
          aria-label="AeroCare AI"
        >
          <span className="inline-flex">
            {'AeroCare'.split('').map((char, i) => (
              <span key={`char-ac-${i}`} className="inline-block overflow-hidden py-1">
                <span className="hero-title-char inline-block will-change-transform">
                  {char}
                </span>
              </span>
            ))}
          ...
          <span className="inline-flex text-emerald-500">
            {'AI'.split('').map((char, i) => (
              <span key={`char-ai-${i}`} className="inline-block overflow-hidden py-1">
                <span className="hero-title-char inline-block text-emerald-500 will-change-transform">
                  {char}
                </span>
              </span>
            ))}
        ...
        <h2
          id="hero-headline"
          ref={headlineRef}
          className="text-3xl sm:text-5xl md:text-6xl font-light leading-[1.1] tracking-tight text-white max-w-3xl mx-auto mb-6 will-change-transform"
        >
          ...
          <span className="block sm:inline overflow-hidden py-0.5">
            <span className="hero-headline-l2 block sm:inline text-zinc-500 will-change-transform">
        ...
        <p
          ref={subtitleRef}
          className="text-sm sm:text-base md:text-lg text-zinc-400 max-w-2xl mx-auto mb-10 leading-relaxed font-light opacity-0 will-change-transform"
        >
        ...
        {/* Quick Atmospheric Snapshot Widget in Elegant Dark theme */}
        <div
          id="hero-snapshot-card"
          ref={snapshotCardRef}
          className="w-full max-w-2xl bg-zinc-950/25 border border-white/[0.08] rounded-2xl p-6 backdrop-blur-xl mb-10 shadow-[0_18px_60px_rgba(0,0,0,0.22)] relative overflow-hidden opacity-0 will-change-transform"
        ...
        >
          <div className="flex items-center justify-between border-b border-zinc-800/60 pb-3 mb-4">
            <div className="flex items-center gap-2">
              ...
              {currentCity.isRealTelemetry && (
                <span className="text-[9px] uppercase tracking-wider font-mono text-emerald-400 bg-emerald-500/10 px-1.5 py-0.5 rounded border border-emerald-500/20 flex items-center gap-1">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-left">
            <div className="hero-snapshot-item flex flex-col">
              <span className="text-2xl sm:text-3xl font-light text-zinc-100">{displayTemp}</span>
              <span className="text-[10px] uppercase tracking-widest text-zinc-500 font-bold mt-1">Temperature</span>
            </div>
            <div className="hero-snapshot-item flex flex-col">
              <span className="text-2xl sm:text-3xl font-light text-zinc-100">{currentCity.humidity}%</span>
              <span className="text-[10px] uppercase tracking-widest text-zinc-500 font-bold mt-1">Humidity</span>
            </div>
            <div className="hero-snapshot-item flex flex-col">
              <span className="text-2xl sm:text-3xl font-light text-emerald-400">{currentCity.aqi} AQI</span>
              <span className="text-[10px] uppercase tracking-widest text-zinc-500 font-bold mt-1">Air Quality</span>
            </div>
            <div className="hero-snapshot-item flex flex-col">
              <span className="text-2xl sm:text-3xl font-light text-zinc-100">{currentCity.uvIndex} UV</span>
              <span className="text-[10px] uppercase tracking-widest text-zinc-500 font-bold mt-1">Solar Index</span>
            </div>
          </div>
        </div>
        ...
        >
          <button
            ...
            className="w-full sm:w-auto px-8 py-4 bg-white text-black rounded-full text-xs font-bold uppercase tracking-widest hover:bg-emerald-400 transition-colors shadow-lg shadow-white/5 flex items-center justify-center gap-2 cursor-pointer"
          >
          ...
          <button
            ...
            className="w-full sm:w-auto px-8 py-4 bg-white/5 rounded-full border border-white/15 text-zinc-300 hover:bg-white/10 hover:border-white/30 hover:text-white text-xs font-bold uppercase tracking-widest transition-colors flex items-center justify-center gap-2 cursor-pointer"
          >
      ...
      {/* Clear Visual Scroll Indicator matching Design HTML */}
      <div
      ...
      >
        ...
        >
          <div className="w-10 h-10 rounded-full border border-zinc-700 flex items-center justify-center group-hover:border-emerald-500 transition-colors relative">
            <div className="w-2 h-3.5 border-b-2 border-r-2 border-emerald-500 rotate-45 mb-1 animate-bounce" />
            <div className="absolute inset-0 rounded-full border border-emerald-500/20 animate-beacon-pulse pointer-events-none" />
          </div>
          <span className="text-xs text-zinc-400 group-hover:text-emerald-400 uppercase tracking-widest transition-colors">
            Explore Analysis
          </span>
        </button>
      </div>
